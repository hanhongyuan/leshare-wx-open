...
import shop.leshare.weixin.open.bean.wx.*;
import shop.leshare.weixin.open.manage.RedisStringManage;
import shop.leshare.weixin.open.mapper.WxOpenUserMapper;
...
public class WxOpenServiceImpl implements WxOpenService{
	...
	/**
	 * post数据请求, URL中必须包含componentAccessToken
	 * @param url
	 * @param body
	 * @return
	 * @throws WxErrorException
	 */
	public String post(String url, String body) throws WxErrorException {
		
		String json = null;
		try {
			json = SimplePostRequestExecutor.create(wxMpService.getRequestHttp()).execute(String.format(url, this.queryComponentAccessToken()), body);
		} catch (IOException e) {
			logger.error(e.toString(), e);
		}
		...
		if(StringUtils.isEmpty(json)){
			...
			throw new WxErrorException(WxError.newBuilder().setErrorCode(9999).setErrorMsg("post请求失败.").build());
		}
		
		logger.info("post请求, url:{}, body:{}, resp:{}", url, body, json);
	...
	public String queryPreAuthCode() throws WxErrorException {
		...
		String preAuthCodeJson = post(API_CREATE_PREAUTHCODE_URL, appidQuery.toJson());
...
//		redisStringManage.addString(PRE_AUTH_CODE_KEY, preAuthCodeResult.getExpires_in(), preAuthCodeResult.getPre_auth_code());
	...
	public String authorizer(String authCode) throws WxErrorException {
		...
		String json = post(API_QUERY_AUTH_URL, authCodeQuery.toJson());
		...
		redisStringManage.addString(authCodeResult.getAuthorization_info().getAuthorizer_appid() + AUTHORIZER_REFRESH_TOKEN_KEY,
				...
				authCodeResult.getAuthorization_info().getAuthorizer_refresh_token());
	...
	public Result saveUserInfo(String appId) throws WxErrorException {
		...
		String json = post(API_GET_AUTHORIZER_INFO, authInfoQuery.toJson());
		...
		WxOpenAuthInfoResult authInfoResult = WxOpenAuthInfoResult.fromJson(json);
		
		OpenUser user = this.authInfoResultToOpenUser(authInfoResult);
